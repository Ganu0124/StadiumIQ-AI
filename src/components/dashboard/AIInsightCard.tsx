...
export function AIInsightCard({ onActionExecute }: AIInsightCardProps) {
  return (
    <section aria-label="Gemini Operations Co-Pilot AI Insights" className="h-full">
      <Card variant="glass" className="h-full flex flex-col border border-primary/20 shadow-glow min-h-[350px]">
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-border">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-md bg-primary-muted flex items-center justify-center text-primary" aria-hidden="true">
        ...
        {/* Insights List */}
        <div 
          className="flex-1 overflow-y-auto mt-4 space-y-4 pr-1 max-h-[380px] focus-visible:ring-1 focus-visible:ring-primary rounded-md"
          tabIndex={0}
          aria-label="AI Operations Insights"
        >
                ...
                <div className="flex flex-wrap gap-2 pt-2 border-t border-white/[0.03]">
                  {insight.actions.map((act) => (
                    <Button
                      ...
                      aria-label={`${act.label} - confidence ${insight.confidence}%`}
                    >
